...
from pptx.enum.text import MSO_AUTO_SIZE, PP_ALIGN, MSO_VERTICAL_ANCHOR # <-- MSO_VERTICAL_ANCHOR was added
...
# ==============================================================================
# === REPLACED AND IMPROVED FUNCTION BELOW =====================================
# ==============================================================================

def add_content_slide_layout(prs, title, content_chunk, image_stream=None):
    """
    Adds a content slide with a modern, professional, and visually balanced layout.

    This function creates a slide with a distinct header bar for the title,
    optimizes font sizes for readability, and intelligently places an image
    as a dynamic, full-height element.
    """
    slide = prs.slides.add_slide(prs.slide_layouts[6]) # Using a blank layout for custom design

    # Set a subtle background color for the entire slide
    # You can change RGBColor(240, 240, 240) to any color you like
    background = slide.background
    fill = background.fill
    fill.solid()
    fill.fore_color.rgb = RGBColor(240, 240, 240)

    # --- Create a professional header bar for the title ---
    # This shape acts as a background for the title text
    header_shape = slide.shapes.add_shape(
        1,  # 1 corresponds to a rectangle shape
        Inches(0), Inches(0), prs.slide_width, Inches(1.0)
    )
    header_fill = header_shape.fill
    header_fill.solid()
    # You can change the header color here
    header_fill.fore_color.rgb = RGBColor(0, 51, 102) # A deep blue
    header_shape.line.fill.background() # No border on the shape

    # Add the title text on top of the header bar
    title_box = slide.shapes.add_textbox(Inches(0.4), Inches(0), prs.slide_width - Inches(0.8), Inches(1.0))
    ...
    content_top_margin = Inches(1.2)
    content_bottom_margin = Inches(0.5)
    content_height = prs.slide_height - content_top_margin - content_bottom_margin

    if image_stream:
        # Layout with an image: Text on the left, full-height image on the right
        # Image takes up 40% of the slide width
        image_width = prs.slide_width * 0.40
        image_left = prs.slide_width - image_width

        try:
            # Add the image, making it cover the full height of the content area
            image_stream.seek(0)
            pic = slide.shapes.add_picture(
                image_stream, image_left, content_top_margin, width=image_width, height=content_height
            )
        except Exception as e:
            st.error(f"Could not add image: {e}")

        # Add the text content in the remaining space on the left
        text_width = image_left - Inches(0.8)
        text_box = slide.shapes.add_textbox(Inches(0.4), content_top_margin, text_width, content_height)
        tf_body = text_box.text_frame
        tf_body.text = content_chunk
        tf_body.vertical_anchor = MSO_VERTICAL_ANCHOR.MIDDLE # Key for good spacing
        tf_body.word_wrap = True
        # Using SHAPE_TO_FIT_TEXT can sometimes be unpredictable with vertical centering.
        # We manually manage font size for more control.
        # tf_body.auto_size = MSO_AUTO_SIZE.SHAPE_TO_FIT_TEXT

        # Set font properties for the body text
        for paragraph in tf_body.paragraphs:
            paragraph.font.name = 'Helvetica'
            paragraph.font.size = Pt(16) # Readable font size
            paragraph.line_spacing = 1.5 # More space between lines

    else:
        # Layout without an image: Text takes up the full width
        text_width = prs.slide_width - Inches(0.8)
        text_box = slide.shapes.add_textbox(Inches(0.4), content_top_margin, text_width, content_height)
        tf_body = text_box.text_frame
        tf_body.text = content_chunk
        tf_body.vertical_anchor = MSO_VERTICAL_ANCHOR.MIDDLE
        tf_body.word_wrap = True
        
        # Set font properties for the body text
        for paragraph in tf_body.paragraphs:
            paragraph.font.name = 'Helvetica'
            paragraph.font.size = Pt(20) # Larger font when no image
...
def main():
    ...
    Content: [A final summary or a call to action.]
